...
export function RaoulPhotoUploader({ invitationId, photos, coverPhotoUrl }: Props) {
  const router = useRouter();
  const [error, setError] = useState<string | null>(null);
  const [busySlot, setBusySlot] = useState<RaoulPhotoSlotId | "extra" | null>(null);
  const [extraUploadLabel, setExtraUploadLabel] = useState<string | null>(null);
  const [removingId, setRemovingId] = useState<string | null>(null);
  const extraInputRef = useRef<HTMLInputElement>(null);

  const classified = useMemo(
    () => classifyRaoulDashboardPhotos(photos, coverPhotoUrl),
    [photos, coverPhotoUrl]
  );

  const slotPhoto: Record<Exclude<RaoulPhotoSlotId, "extra">, Photo | null> = {
  ...
  const slotUrl: Record<Exclude<RaoulPhotoSlotId, "extra">, string | null> = {
  ...
  async function uploadToSlot(
    slotId: Exclude<RaoulPhotoSlotId, "extra">,
    file: File
  ) {
    const slot = RAOUL_PHOTO_SLOTS.find((s) => s.id === slotId)!;
  ...
  async function removeSlot(slotId: Exclude<RaoulPhotoSlotId, "extra">) {
    const existing = slotPhoto[slotId];
    ...
    if (!existing) return;
    if (!confirm(`Hapus foto ${RAOUL_PHOTO_SLOTS.find((s) => s.id === slotId)?.label}?`)) return;
