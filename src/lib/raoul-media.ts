...
/** Theme defaults — Raoul & Christine daylight set */
...
const GROOM_CAPTION_KEYS = ["mempelai pria", "groom", "pria", "raoul"];
...
export const RAOUL_PHOTO_SLOTS = [
  {
    id: "hero" as const,
    label: "Latar hero",
...
export function resolveRaoulMedia(invitation: {
  ...
  const photos = hasCustomPhotos ? invitation.photos : raoulDefaultPhotos;

  const heroBg = invitation.coverPhotoUrl ?? raoulAssets.heroBg;

  let groomPhoto = pickPortrait(photos, GROOM_CAPTION_KEYS, raoulAssets.groomPortrait, new Set());
  let bridePhoto = pickPortrait(
    photos,
    BRIDE_CAPTION_KEYS,
    raoulAssets.bridePortrait,
    new Set([groomPhoto])
  );
  ...
  if (hasCustomPhotos) {
    ...
    if (bridePhoto === raoulAssets.bridePortrait) {
      const next = photos.find((p) => !used.has(p.url));
      if (next) {
        bridePhoto = next.url;
        used.add(bridePhoto);
